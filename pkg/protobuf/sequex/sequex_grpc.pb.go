...
type SequexServiceClient interface {
	OnEvent(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Event, Event], error)
}
...
func (c *sequexServiceClient) OnEvent(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Event, Event], error) {
...
type SequexService_OnEventClient = grpc.BidiStreamingClient[Event, Event]
...
type SequexServiceServer interface {
	OnEvent(grpc.BidiStreamingServer[Event, Event]) error
	mustEmbedUnimplementedSequexServiceServer()
}

// UnimplementedSequexServiceServer must be embedded to have
...
func (UnimplementedSequexServiceServer) OnEvent(grpc.BidiStreamingServer[Event, Event]) error {
...
func RegisterSequexServiceServer(s grpc.ServiceRegistrar, srv SequexServiceServer) {
	// If the following call pancis, it indicates UnimplementedSequexServiceServer was
...
func _SequexService_OnEvent_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(SequexServiceServer).OnEvent(&grpc.GenericServerStream[Event, Event]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type SequexService_OnEventServer = grpc.BidiStreamingServer[Event, Event]
